...
At this point we bind the 3 dataframes subject, ytrain, and xtrain by column to create a temporary dataframe
train, the train dataframe is the full data for all the subjects in the training subject group.  We remove the row, subject, xtrain, ytrain, dataframes/variables as they are no longer needed.
...
From the fulldata we extract 66 columns for the mean and standard deviation values, storing them into a
datatable.  Column/variable names are converted to a more tidy format, ordered, averages calculated, and written out 
to a text data file called tidydataset.txt in the working directory using write.table.
